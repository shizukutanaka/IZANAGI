...
| H50 | **MSRV トリップワイヤが src/bin/ を見ていなかった** — `msrv_is_respected` は `library_sources` 経由で bin を除外 — しかし `gamec` は tarball で出荷され消費者の宣言 MSRV でビルドされる。gate の tarball テストは現行 toolchain で走るため検出不能。bin/*.rs の同等走査を追加し `is_none_or`(1.82>1.75)注入で発火確認。「走査がある」≠「全出荷ターゲットに届く」 |
...
| H55 | **実バグ(2件): `clamp` の NaN 透過による蓄積系の永久汚染** — `f32::clamp` は NaN をそのまま返すため `Time::advance(NaN)` は `elapsed` を永久 NaN 化、`Camera::follow(NaN dt)` は `pos` を永久 NaN 化。`max(lo).min(hi)` は NaN を「欠損の境界」として扱い 0/lo に畳む。両箇所を修正し回帰テスト追加(NaN/±inf/負)。一発計算の clamp(ease, 色)は NaN-in→NaN-out が妥当で対象外 |
| H56 | **実バグ(同型): Tween/Timer/Animation の NaN dt 蓄積** — `Timer::tick(NaN)` は `elapsed` を永久 NaN(NaN>=duration は偽で二度と発火しない)、`Tween` は NaN で即時完了(min が NaN を duration に畳む)、`Animation` は永久凍結。3系とも `!dt.is_finite()` で 1 フレーム no-op に統一 — 経路ごとに違う失敗形(発火しない/即完了/凍結)が同じ原因から出る。±inf も対象(repeating timer は inf で永久発火) |
| H57 | **実バグ(同型第3波): Metrics::record の `dt <= 0.0` ガードは NaN を通す**(NaN<=x は偽) — sum_dt/elapsed が永久 NaN。`!is_finite() || <= 0.0` で拒否(`!(dt>0)` は clippy neg_cmp_op_on_partial_ord に抵触)。audio の volume/master・gamepad の stick/trigger の `clamp` も伝播するため is_nan ガードに(蓄積系では neutral=0 に畳む)。**比較ガードの不等号方向が NaN 安全性を決める** — 条件を否定形で書くか肯定形で書くかは等価でない |
