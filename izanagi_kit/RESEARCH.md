...
| N5 | **MapBuilder パイプライン**(cellular→drunkard→prefab→post-filter の合成)。**階段配置 `farthest_cell` は実装済み**: `pathfinding::farthest_cell(sources, max_cost, is_blocked)` — dijkstra_map の argmax(row-major tie-break で決定論的)、Dungeon に `\|x,y\| d.is_wall(x,y)` で直結。合成層本体は Dungeon の可変 API 設計が必要で未着手 | Wolverson RC 2020 / Brogue RC 2018 | 🟡 新 module | ジェネレータ合成層は設計要 |
