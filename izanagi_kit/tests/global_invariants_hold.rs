...
/// in the same commit. (Baseline: 23 of 89 impl'd kit types pinned. Round 66
/// pinned every publicly-constructible type — 97 golden cases total.)
const UNPINNED_DET_HASH: &[&str] = &[
    // `MonitorState` has private fields and is only produced by stepping a
    // `Monitor` — there is no canonical public fixture for it.
    "MonitorState",
];
