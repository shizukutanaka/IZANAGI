...
| G11 | **条件コンパイルでターゲット・プロファイル・feature に分岐せず、ビルドへの外部入力を列挙しきる**（library code の `cfg`/`cfg!`/`cfg_attr` 判別式は `test`/`doc`/`doctest`/`docsrs` と `not`/`any`/`all` のみ。manifest の `target`/`features`/`lints`/`patch`/`replace`/`build-dependencies` テーブルと `[[bin]]`/`[[test]]`/`[[bench]]`/`[[example]]` ターゲットテーブル、`harness`/`auto*`/`crate-type`/`proc-macro`/`test`/`bench`/`doctest` キー、profile テーブルの `debug-assertions`/`overflow-checks`/`panic` キー、`.cargo` 設定・`rust-toolchain`/`Cross.toml` ファイルを禁止。出荷コードは `env!`/`option_env!`/`extern`/`#[no_mangle]`/`#[link` を使わない。gate.sh は RUSTFLAGS/RUSTDOCFLAGS を除去してから走る） | 「値」ではなく「コンパイルされるコードそのもの」がターゲットやプロファイルで変わる経路。`#[cfg(target_pointer_width)]` は禁止トークンを一切使わず G4/G9/G10 を迂回できる |
