...
| G11 | **条件コンパイルでターゲット・プロファイル・feature に分岐しない**（library code の `cfg`/`cfg!`/`cfg_attr` 判別式は `test`/`doc`/`doctest`/`docsrs` と `not`/`any`/`all` のみ。manifest の `target`/`features`/`lints`/`patch`/`replace`/`build-dependencies` テーブルと `.cargo` 設定ファイルを禁止） | 「値」ではなく「コンパイルされるコードそのもの」がターゲットやプロファイルで変わる経路。`#[cfg(target_pointer_width)]` は禁止トークンを一切使わず G4/G9/G10 を迂回できる |
