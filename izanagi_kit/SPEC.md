...
| G11 | **条件コンパイルでターゲット・プロファイル・feature に分岐せず、ビルドへの外部入力を列挙しきる**（library code の `cfg`/`cfg!`/`cfg_attr` 判別式は `test`/`doc`/`doctest`/`docsrs` と `not`/`any`/`all` のみ — `cfg_attr` の適用側にネストした `cfg`/`cfg_attr`/`cfg!` も再帰的に同じ許可リストで検査。manifest の `target`/`features`/`lints`/`patch`/`replace`/`build-dependencies` テーブルと `[[bin]]`/`[[test]]`/`[[bench]]`/`[[example]]` ターゲットテーブル、`harness`/`auto*`/`crate-type`/`proc-macro`/`test`/`bench`/`doctest`/`links` キー、`[lib]`/`[bin]`/`[test]`/`[bench]`/`[example]` の `path` 差し替え（kit の `[lib]` は `path = "src/lib.rs"` に固定）、profile テーブルの `debug-assertions`/`overflow-checks`/`panic` キー、`.cargo` 設定・`rust-toolchain`/`Cross.toml` ファイルを禁止。出荷コードは `env!`/`option_env!`/`extern`/`#[no_mangle]`/`#[link` を使わない。gate.sh は実行前に `CARGO*`/`RUST*`/`RUSTUP*` 系環境変数をすべて除去し、`CARGO_HOME` をリポジトリ内 `target/` に固定してから走る — `RUSTC_BOOTSTRAP`/`CARGO_ENCODED_RUSTFLAGS`/`CARGO_BUILD_RUSTFLAGS`/`RUSTUP_TOOLCHAIN` 等の同等注入経路も塞ぐ） | 「値」ではなく「コンパイルされるコードそのもの」がターゲットやプロファイルで変わる経路。`#[cfg(target_pointer_width)]` は禁止トークンを一切使わず G4/G9/G10 を迂回できる。環境変数も同じ力を持つ — `--cap-lints` や nightly feature gate を呼び出し側が黙って差し込めた |
| G12 | **ワークスペース全体で unordered container を使わない**（`HashMap`/`HashSet`/`hash_map`/`hash_set`/`DefaultHasher`/`RandomState`/`SipHash`/`BuildHasher` を library code・engine src のいずれにも持たない） | 反復順がプロセス毎のランダム hasher seed で決まり、その順序に依存する fold（audio mixer の `f32` 加算等）は同じ入力でも run 毎に bit ずれ得る。kit は allowlist で既に管理、engine は「despawn でのみ反復する HashMap」例外が実際には `mix_into` の voices にも存在したため、例外管理ではなく型の不在で強制 |
