...
| `xml` / `mp4` / `h2` / `vcdiff` / `pcapng` / `pgp` / `bdf` | Document, media, transport, delta, capture, mail-crypto, and font formats: XML 1.0 (`parse`/`escape`/`unescape` — well-formedness only: matched tags, one root, entity/CDATA decode, `Ev` stream; not DTD-validating), MP4/ISOBMFF (`boxes`/`children`/`ftyp`/`mvhd`/`tkhd`/`mdhd`/`hdlr`/`stts`/`find` — `size:u32 type:u32` walk, largesize/`size==0`/`uuid`, `meta`+4 and `stsd`+8 container headers, v0/v1 normalized), HTTP/2 (`PREFACE`/`frames`/`emit`/`payload`/`kind_name`/`settings` — 9-byte headers, all 10 frame types, PADDED/PRIORITY stripping, 2^14 default cap), VCDIFF (`apply`/`emit_add` — RFC 3284 window decoder: generated 256-entry code table, ADD/RUN/COPY, NEAR/SAME address cache, VCD_HERE/SELF/TARGET addressing), pcapng (`parse`/`options`/`kind_name`/`packet`/`Epb::ts_ns` — palindromic SHB type, per-section endianness from BOM, IDB/EPB/SPB, `if_tsresol` to ns), OpenPGP (`packets`/`payload`/`tag_name`/`armor`/`unarmor`/`crc24` — RFC 9580 new+legacy packet headers, partial-body chains, ASCII armor + CRC-24), and Adobe BDF (`parse`/`Glyph::pixel`/`glyph_by_encoding`/`glyph_by_name` — STARTFONT…ENDFONT, per-glyph SWIDTH/DWIDTH/BBX, MSB-first hex bitmaps). |
