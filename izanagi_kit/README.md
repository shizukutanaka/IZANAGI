...
| `threat` / `pool` / `tween` / `wallet` / `shop` / `dialogue` | Per-combatant aggro tables, bounded regenerating resources (mana/stamina), eased time-driven value interpolation, fungible currency wallets, wallet-backed buy/sell price listings, branching NPC conversation trees. |
