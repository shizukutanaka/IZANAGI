...
| `ttf` / `woff` / `sqlite` / `proto` / `bson` / `flac` / `ogg` | Font, database, serialization, and media-container formats: sfnt/TrueType (`parse`/`table`/`head`/`maxp`/`names` — offset table + directory, `head`/`maxp`/`name` typed views), WOFF (`parse`/`inflate_table` — `wOFF` header + 20-byte directory, deflate-compressed entries via `inflate`), SQLite3 (`parse`/`page1_tree`/`page1_cells` — the 100-byte header, page size/encoding/schema cookie, page-1 b-tree walk), Protocol Buffers wire (`tag`/`varint`/`len_field`/`fixed64_field`/`fixed32_field`/`fields`/`varint_at`/`bytes_at`/`string_at`/`fixed64_at`/`fixed32_at` — `(num<<3)|wire` tag stream, varint/fixed/LEN spans, groups rejected), BSON (`parse`/`emit`/`Val`/`get` — i32-length documents, all element types, IEEE payloads kept as raw bits), FLAC (`parse`/`streaminfo`/`comments` — `fLaC` + metadata chain, STREAMINFO packed fields, Vorbis comments), and Ogg (`pages`/`packets`/`emit_page`/`ogg_crc` — `OggS` walk, non-reflected CRC-32 verification, packet reassembly across pages). |
| `packfile` / `classfile` / `ar` / `plist` / `shp` / `pcapng` / `mbox` | VCS/VM/archive/config/geo/capture/mail containers: git packfile (`parse`/`parse_idx`/`find` — `PACK` v2/3 object headers, OFS/REF delta bases, zlib members via `inflate_zlib_count`, `.idx` v2 fanout lookup), Java `.class` (`parse`/`utf8`/`class_name` — `CAFEBABE` constant pool with 2-slot Long/Double, field/method/attribute tables), Unix ar archive (`parse`/`name`/`data`/`find` — `!<arch>` 60-byte headers, GNU `//` string-table names, BSD `#1/` inline names), Apple plist (`parse_bin`/`resolve`/`parse_xml`/`emit_xml`/`get` — `bplist00` object table + trailer, XML `<plist>` tree, raw IEEE bits for reals/dates), ESRI `.shp` (`parse`/`point`/`points`/`f64_fixed`/`f64_bits` — BE file header + LE records, doubles decoded to `Fixed` by hand), pcapng (`parse`/`packets` — endian-aware section blocks, IDB/EPB extraction), and mbox (`parse`/`headers`/`body` — `From ` line splits, `>From ` stays verbatim). |
| `dbf` / `iso9660` / `mvt` / `pgp` / `ply` / `fits` / `qcow2` | Attribute-table, filesystem, vector-tile, crypto-packet, mesh, astronomy, and VM-disk formats: dBASE III (`parse`/`cell`/`field_name`/`deleted` — 32-byte header + descriptor chain, flag-prefixed records), ISO 9660 (`parse`/`entries`/`find`/`both32`/`dir_rec` — sector-16 PVD, both-endian 733 fields, directory extent walk), Mapbox Vector Tile (`parse`/`geom`/`attrs`/`zigzag` — `proto`+`inflate_gzip` composition, MoveTo/LineTo/ClosePath command stream), OpenPGP (`packets`/`body`/`dearmor`/`crc24`/`tag_name` — RFC 4880 new/old length forms, partial-body chunking, ASCII armor), PLY (`parse`/`cell`/`tokens` — ascii + both-endian binary rows, list properties, raw IEEE bits), FITS (`parse`/`get`/`get_int` — 80-byte cards in 2880 blocks, `BITPIX × NAXISn` data math, multi-HDU), and QCOW2/3 (`parse`/`cluster_size`/`l1_needed`/`incompat` — `QFI\xFB` BE header, backing path, refcount/L1/snapshot offsets). |
| `cab` / `fat` / `jpeg` / `icns` / `ttc` / `bdf` / `pdf` | Archive, filesystem, image, icon, font, bitmap-font, and document formats: MS-CAB (`parse`/`folder_idx`/`folder_bytes`/`extract` — MSCF header, reserved areas, Stored/MSZIP `CK`+DEFLATE folders), FAT (`parse`/`fat_entry`/`eoc`/`chain`/`read_chain`/`dir_slot`/`entries`/`entry_name`/`file_bytes`/`find` — BPB geometry, FAT12/16/32, packed FAT12 entries), JPEG (`parse`/`app`/`is_sof`/`standalone` — marker-segment walk, entropy scan with FF00-stuffing and RSTn handling, APPn prefix select), ICNS (`parse`/`image`/`is_png`/`is_jp2`/`kind`/`entry_name` — `icns` container entries, PNG/JP2/RGB/mask tags), TTC (`parse`/`fonts`/`font`/`is_collection` — `ttcf` collection header, offset table, v1/v2 DSIG), BDF (`parse`/`glyph`/`row`/`row_at`/`bit`/`render` — STARTFONT/CHARS blocks, BBX metrics, MSB-first hex bitmaps), and PDF (`parse`/`obj_at`/`obj`/`dict_get`/`root`/`pages`/`page_count`/`page_ids` — `%PDF` header, `startxref`+classic xref table, trailer/Root/Pages walk, exact-decimal reals — xref streams out of scope). |
| `ebml` / `isobmff` / `aiff` / `xpm` / `gltf` / `wad` / `svg` | Media, model, and game-asset containers: EBML (`parse`/`children`/`uint`/`int`/`text`/`utf8`/`float_bits`/`date`/`vint`/`find`/`find_all`/`is_master`/`data` — RFC 9559 VINT ids/sizes, unknown-size masters, Matroska/WebM element walk), ISO BMFF (`parse`/`children`/`find`/`find_path`/`major_brand`/`compatible_brands`/`mvhd`/`tkhd`/`stts`/`stsz`/`chunk_offsets` — box tree, largesize+uuid headers, `meta` flags skip), AIFF/AIFC (`parse`/`comm`/`ssnd`/`chunk`/`extended80` — FORM chunks, 80-bit extended rate → `Fixed`), XPM (`parse`/`pixel`/`symbol`/`transparent`/`quoted_lines` — XPM3 quoted lines, multi-char symbols, `c` key color rows), glTF GLB (`parse`/`chunk_data`/`json_bytes`/`json_len`/`bin`/`json` — `glTF` v2 header, JSON chunk delegated to `json`), Doom WAD (`parse`/`is_iwad`/`lump`/`find`/`find_name`/`find_all`/`block` — IWAD/PWAD directory, uppercase 8-byte names, marker blocks), and SVG path data (`parse`/`bbox`/`Seg`/`P2` — M/L/H/V/C/S/Q/T/A/Z abs+rel, reflection-resolved S/T, exact-decimal `Fixed` coords). |
| `vox` / `dds` / `modfile` / `chip8` / `ines` / `tap` / `ips` | Retro game assets, consoles, and dev-tool formats: MagicaVoxel `.vox` (`parse`/`palette`/`rgba`/`color`/`DEFAULT_PALETTE` — `VOX` magic, MAIN chunk walk, SIZE/XYZI/RGBA, the 0xAABBGGRR default palette), DirectDraw Surface headers (`parse`/`block`/`data`/`is_compressed`/`is_cubemap`/`is_volume`/`top_mip_size`/`PixelFormat`/`Dx10Header` — 124-byte header, FourCC vs RGB layouts), ProTracker `.mod` (`parse`/`pattern`/`row`/`sample_data`/`amiga_hz_x100`/`PERIODS` — 15/31-sample layouts, period table, 4-byte event cells), a CHIP-8 interpreter (`Chip8`/`step`/`press`/`run_frame`/`pixels`/`COSMAC`/`MODERN` — 4K memory, timers, XOR sprites, both quirk sets), iNES/NES 2.0 ROM headers (`parse`/`prg_at`/`chr_at`/`end`/`fits`/`Nes`/`Mirroring`/`Tv` — exponent-multiplier sizes, PAL, battery, trainers), Test Anything Protocol v14 (`parse`/`summary`/`ok`/`plan`/`bail_out`/`skipped_all`/`version` — plan, points, TODO/SKIP directives, YAML blocks), and IPS patches (`parse`/`emit`/`apply`/`diff` — u24 offsets, RLE records, Lunar IPS truncation extension, minimal patch generation). |
| `nsf` / `gbs` / `psid` / `spc` / `vgm` / `psf` / `figlet` | Retro game music and font formats: NSF NES music (`parse`/`region`/`has_chip`/`is_banked`/`end`/`fits`/`Nsf`/`Region`/`Chip` — 128-byte header, load/init/play addrs, expansion audio bitmask, bankswitch init), GBS Game Boy music (`parse`/`uses_timer`/`timer_hz`/`data_at`/`Gbs` — 112-byte header, TMA/TAC timer playback), PSID/RSID SID music (`parse`/`kind`/`uses_cia`/`sid_model`/`file_load`/`Psid`/`Kind`/`SidModel` — big-endian v1/v2 headers, embedded load addresses, multi-SID flag decoding), SPC SNES snapshots (`parse`/`ram`/`dsp`/`ipl`/`Spc`/`Tag`/`TagKind` — PC/A/X/Y/PSW/SP register restore, ID666 text vs binary tags), VGM video game music logs (`parse`/`commands`/`gd3`/`Vgm`/`Cmd`/`RATE` — EOF/loop/data offsets, wait+write command stream walk, UTF-16 GD3 tags), PSF1/PSF2 console fonts (`parse`/`version`/`glyph`/`unicode`/`Psf`/`Version` — 512-glyph mode, UTF-16LE mapping table walk), and FIGlet `.flf` fonts (`parse`/`glyph`/`render`/`Figlet`/`STANDARD` — `flf2a` header, 95 standard glyphs, hardblank/endmark stripping, horizontal smushing render). |
| `gb` / `gba` / `z64` / `sfc` / `fds` / `tzx` / `d64` | Retro machine ROM and media images: Game Boy cartridges (`parse`/`Gb`/`Cgb`/`Cart`/`Dest`/`NINTENDO_LOGO`/`HEADER` — 0x100 entry, 48-byte logo check, title/CGB/SGB flags, cartridge type, ROM/RAM size codes, header+global checksums), Game Boy Advance (`parse`/`checksum`/`Gba`/`NINTENDO_LOGO` — 156-byte compressed logo, game/maker codes, fixed 0x96 byte, complement checksum), Nintendo 64 ROMs in all three endian layouts (`parse`/`byte_at`/`magic_of`/`unswap_v64`/`Z64`/`ByteOrder` — z64/v64/n64 magic sniffing, PC/clock/CRC/name/serial decode), Super Famicom/SNES images (`parse`/`Sfc`/`MapMode`/`COPIER` — 512-byte copier-header detection, LoROM/HiROM/ExHiROM header scoring by checksum^complement, map mode, title), Famicom Disk System disks (`parse`/`side`/`Fds`/`SIDE` — `FDS\x1A` fwNES header or raw 65500-byte sides), ZX Spectrum TZX tapes (`parse`/`blocks`/`Tzx`/`Block`/`ID` — `ZXTape!` signature, per-block-id length table, iterator over the block stream), and Commodore 1541 D64 disk images (`parse`/`sector_at`/`disk_name`/`free_sectors`/`dir`/`petscii`/`D64`/`Entry`/`Kind` — per-track sector geometry, BAM free counts, PETSCII names, directory chain walk). |
