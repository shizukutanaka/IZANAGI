...
- ~~panic したらテストは落ちる~~ → `#[should_panic]` 無印はどんな panic でも
  緑 — 弱テストは「ピン要件のない属性」として機械検査で弾く。
- ~~ハッシュのピンがあれば impl の正しさは十分~~ → ピンは1点の定点検査 —
  フィールドを読まない退化は fixture が動かなければ検出不能。
- ~~ライブラリが MSRV を守れば出荷物は安全~~ → bin は別走査領域 —
  出荷される全コンパイル単位が検査に届くかを問う。
- ~~逆変換の往復テストは変換の正しさを証明する~~ → 往復は逆写像性のみ —
  両側が同じ誤りを持つ鏡像変異は通過する。方向の定点ピンが必要。
- ~~正座標の変換テストで丸めは検証済み~~ → floor と trunc が分岐するのは
  負側だけ — 意味論の違いが出る領域でテストする。
- ~~エッジ検出は押下時の発火を確認すれば十分~~ → 「無いはずのエッジが
  出ない」も契約 — ガードの戻り値が意味を持つ箇所は負側をピンする。
- ~~呼べば panic しないことは意味の検証になる~~ → `let _ =` は値を捨てる
  — doc 契約の値は別途 assert でピンする。
- ~~clamp(lo,hi) は常に値を範囲内に収める~~ → Rust の clamp は NaN を透過
  — 蓄積系では max().min() を使う。(→ is_nan ガードに改訂: clippy manual_clamp が素朴形を嫌う)
- ~~時間系の dt は常に有限~~ → バックエンド由来の NaN/inf は蓄積変数を
  永久汚染する — 入口で非有限を畳む。
- ~~範囲ガード `x <= 0 return` は異常値を全て弾く~~ → NaN は全比較が偽
  なので否定形(`!(x > 0)`)でないと素通りする。
