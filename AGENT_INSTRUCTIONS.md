...
- ~~ニードルは字句形固定で刺せば足りると思われていた~~ → Rust は `::` `!`
  `.` `#` の周辺と `(`/`[` の直前に空白を許すため、`std :: env :: var`
  `env ! (..)` `cfg ! (unix)` `# [cfg]` が全走査を緑で通過(注入実証)。
  `library_sources`/`test_code` にシジル隣接空白の正規化を追加し、
  `env_macro_args` は `env`/`option_env`→`!`→`(` の空白耐性マッチへ。
  全7コピーはバイト同一ピンで同期済み。なお `fs::` はテストの fixture
  読みで規約上許可のため対象外。
...
### サブモジュール宣言も拒否(2026-09)

~~`.gitmodules` + gitlink で丸ごと走査外のコードをツリー内に置ける~~ — *finding:* submodule の中身は本リポのスキャン対象外のディレクトリに住む(コンパイルされる形はモジュールセンサスが捕捉するが、ファイルの存在自体は未検査だった)。*fix:* `.gitmodules` を root/member 双方で拒否リストへ追加(注入で赤確認)。
...
### `.rs` の居所を全列挙で固定(2026-09)

~~走査対象ディレクトリ外の `.rs` は auto-discovery の有無を問わず「スイートが読まないコード」~~ — *finding:* `benches/`(`ffd41ef`)と同型の面が将来の発見 dir・`fuzz/` 等にも存在し得た。*fix:* リポ全体を再帰走査し `.rs` の存在するディレクトリ集合を既知7箇所にピン(`fuzz/x.rs` 注入で赤確認)。
